<ul aria-label="Social links" class="flex items-center gap-[6px]">
    <li class="h-[30px] w-[30px]">
        <a
                title="GitHub"
                class="font-medium self-start transition-colors hocus:decoration-2 p-0 inline-flex items-center justify-center h-full w-full rounded-[6px] text-tertiary-txt hocus:bg-accent-dark/[0.1] hocus:no-underline hocus:text-[#333] hocus:dark:text-[#ebebeb]"
                aria-label="GitHub"
                target="_blank"
                rel="noopener noreferrer"
                href="https://github.com/customentity"
        >
            <svg viewBox="0 0 24 24" style="width:1.35rem;height:1.35rem" role="presentation">
                <path
                        d="M15.8 22.5c-.6 0-1-.4-1-1V18c0-.7-.2-1.2-.6-1.7-.3-.3-.4-.7-.2-1 .1-.4.5-.6.8-.7 2.6-.3 4.9-1.1 4.9-5.3 0-1-.4-2-1.1-2.7-.3-.3-.3-.7-.2-1 .2-.7.3-1.4.1-2-.4.1-1.2.4-2.3 1.1-.2.2-.5.2-.8.1-1.9-.5-3.9-.5-5.8 0-.1.1-.4.1-.6-.1-1.1-.7-1.9-1-2.3-1.1-.1.7-.1 1.4.1 2 .1.4.1.8-.2 1-.7.8-1.1 1.7-1.1 2.7 0 3.9 2 5 4.9 5.3.4 0 .7.3.8.7.1.4 0 .8-.2 1-.4.4-.6 1-.6 1.6v3.5c0 .6-.4 1-1 1s-1-.4-1-1V20c-2.7.4-3.9-1.1-4.7-2.1-.3-.4-.6-.8-.9-.9-.5-.1-.9-.7-.7-1.2.1-.5.7-.9 1.2-.7.9.2 1.5.9 2 1.6.7.9 1.3 1.7 3.1 1.3 0-.5 0-1.1.2-1.6-2.5-.6-5.2-2.2-5.2-7.1.1-1.3.6-2.6 1.4-3.6-.3-1.2-.2-2.4.3-3.6.1-.3.3-.5.6-.6.3-.1 1.6-.3 4 1.2 2-.5 4-.5 6 0 2.4-1.5 3.7-1.3 4-1.2.3.1.5.3.6.6.5 1.1.6 2.4.3 3.6.8 1 1.2 2.3 1.2 3.6 0 5.1-3.1 6.6-5.2 7.1.2.5.3 1.1.2 1.7v3.4c0 .6-.4 1-1 1z"
                        style="fill:currentColor"
                />
            </svg>
        </a>
    </li>
    <li class="h-[30px] w-[30px]">
        <a
                title="Malt"
                class="font-medium self-start transition-colors hocus:decoration-2 p-0 inline-flex items-center justify-center h-full w-full rounded-[6px] text-tertiary-txt hocus:bg-accent-dark/[0.1] hocus:no-underline hocus:text-[#D4524F] hocus:dark:text-[#D4524F]"
                aria-label="Malt"
                target="_blank"
                rel="noopener noreferrer"
                href="https://www.malt.fr/profile/flaviomoreno"
        >
            <svg
                    xmlns="http://www.w3.org/2000/svg"
                    aria-label="Malt"
                    role="img"
                    viewBox="0 0 512 512"
                    width="24px"
                    height="64px"
                    fill="#000000"
            >
                <g id="SVGRepo_bgCarrier" stroke-width="0"/>
                <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"/>
                <g id="SVGRepo_iconCarrier">
                    <rect width="512" height="512" rx="15%" fill="transparent"/>
                    <path
                            style="fill: currentColor;"
                            d="m408.4 103.8c-32.5-32.4-67.1-11.4-88.8 10.2L114.8 318.8c-21.7 21.7-44.4 54.7-10.2 88.8c34.1 34.1 67 11.4 88.7-10.3l204.8-204.8c21.7-21.6 42.7-56.3 10.3-88.7zm-195.7-8.4 43.4 43.4 44.1-44.2c3-3 6-5.8 9.1-8.4c-4.6-23.3-17.9-44.4-53.3-44.4c-35.4 0-48.7 21.2-53.2 44.5c3.3 2.9 6.6 5.8 9.9 9.1zm87.5 322.1-44.1-44.1-43.4 43.3c-3.3 3.3-6.5 6.4-9.8 9.2c5 23.8 19 45.5 53.1 45.5c34.2 0 48.3-21.9 53.2-45.7c-3-2.6-6-5.2-9-8.2zm-105.9-217h-83.6c-30.7 0-70 9.7-70 55.5c0 34.3 21.9 48.3 45.8 53.2c2.8-3.2 107.8-108.7 107.8-108.7zm231.5 2.3c-2.6 3-107.9 108.8-107.9 108.8h82.4c30.7 0 70-7.3 70-55.6c0-35.3-21.1-48.6-44.5-53.2zm-204.1-29.7 14.9-14.9-43.3-43.4c-21.7-21.7-54.6-44.4-88.8-10.2c-25 25-19.4 49.4-6.2 69.1c4.1-.3 123.4-.6 123.4-.6zm68.7 165.9-15 15 44.2 44.1c21.7 21.7 56.3 42.7 88.7 10.3c24.2-24.2 18.7-49.7 5.3-70c-4.3.3-123.2.6-123.2.6z"
                    />
                </g>
            </svg>
        </a>
    </li>
    <li class="h-[30px] w-[30px]">
        <a
                title="LinkedIn"
                class="font-medium self-start transition-colors hocus:decoration-2 p-0 inline-flex items-center justify-center h-full w-full rounded-[6px] text-tertiary-txt hocus:bg-accent-dark/[0.1] hocus:no-underline hocus:text-[#0077b5] hocus:dark:text-[#1a85bc]"
                aria-label="LinkedIn"
                target="_blank"
                rel="noopener noreferrer"
                href="https://linkedin.com/in/flaviomoreno94/"
        >
            <svg viewBox="0 0 24 24" style="width:1.35rem;height:1.35rem" role="presentation">
                <path
                        d="M21.5 22h-3.8c-.6 0-1-.4-1-1v-6.7c0-.5-.4-.9-.9-.9s-.9.4-.9.9V21c0 .6-.4 1-1 1h-3.8c-.6 0-1-.4-1-1v-6.7a6.7 6.7 0 0 1 13.4 0V21c0 .6-.4 1-1 1zm-2.8-2h1.8v-5.7c0-2.6-2.1-4.7-4.7-4.7s-4.7 2.1-4.7 4.7V20h1.8v-5.7c0-1.6 1.3-2.9 2.9-2.9 1.6 0 2.9 1.3 2.9 2.9V20zM6.3 22H2.5c-.6 0-1-.4-1-1V9.6c0-.6.4-1 1-1h3.8c.6 0 1 .4 1 1V21c0 .6-.4 1-1 1zm-2.8-2h1.8v-9.4H3.5V20zM4.4 7.8c-1.6 0-2.9-1.3-2.9-2.9S2.8 2 4.4 2s2.9 1.3 2.9 2.9S6 7.8 4.4 7.8zm0-3.8c-.5 0-.9.4-.9.9s.4.9.9.9.9-.4.9-.9-.4-.9-.9-.9z"
                        style="fill:currentColor"
                />
            </svg>
        </a>
    </li>
    <li class="h-[30px] w-[30px]">
        <a
                title="Twitter"
                class="font-medium self-start transition-colors hocus:decoration-2 p-0 inline-flex items-center justify-center h-full w-full rounded-[6px] text-tertiary-txt hocus:bg-accent-dark/[0.1] hocus:no-underline hocus:text-[#1881C2] hocus:dark:text-[#34aaf3]"
                aria-label="Twitter"
                target="_blank"
                rel="noopener noreferrer"
                href="https://twitter.com/Flaviomoreno94"
        >
            <svg viewBox="0 0 24 24" style="width:1.35rem;height:1.35rem" role="presentation">
                <path
                        d="M8.6 21.1c-2.4 0-4.8-.6-7.1-1.9-.4-.2-.6-.7-.5-1.1.1-.4.6-.8 1-.7 1.5.1 2.9-.2 4.2-.8C3 14.5 2.1 11.5 2 9.3c-.1-2.7.8-4.9.9-5 .2-.3.5-.5.8-.6.4 0 .7.1.9.4C6.1 6.3 8.5 7.6 11 7.8c0-1.4.6-2.8 1.7-3.8 1.9-1.7 4.8-1.6 6.7 0 .7-.3 1.4-.6 2-1.1.3-.2.8-.2 1.1 0 .3.2.5.7.4 1.1-.3 1.3-.9 2.5-1.8 3.4v.4c0 5-2.2 9.2-6.1 11.5-1.9 1.3-4.1 1.8-6.4 1.8zm-2.7-2.3c2.8.6 5.7.3 8.1-1.2 3.2-1.9 5.1-5.5 5.1-9.8 0-.2 0-.4-.1-.6-.1-.3 0-.7.3-.9l.4-.4c-.1 0-.2.1-.3.1-.4.1-.8 0-1-.3-1.1-1.3-3.1-1.4-4.3-.3-.7.8-1.1 1.6-1.1 2.5v.9c0 .5-.4 1-1 1-3 .1-5.7-1.1-7.8-3.1-.5 2.3-.6 6.6 4.5 8.9.3.1.6.5.6.8 0 .4-.1.7-.4.9-.9.7-1.9 1.2-3 1.5z"
                        style="fill:currentColor"
                />
            </svg>
        </a>
    </li>
    <!--	<li class="h-[30px] w-[30px]">
            <a
                title="Instagram"
                class="font-medium self-start transition-colors hocus:decoration-2 p-0 inline-flex items-center justify-center h-full w-full rounded-[6px] text-tertiary-txt hocus:bg-accent-dark/[0.1] hocus:no-underline hocus:text-[#d6249f] hocus:dark:text-[#da3aa9]"
                aria-label="Instagram"
                target="_blank"
                rel="noopener noreferrer"
                href="https://instagram.com/flavmrno"
            >
                <svg viewBox="0 0 24 24" style="width:1.35rem;height:1.35rem" role="presentation">
                    <path
                        d="M7.8,2H16.2C19.4,2 22,4.6 22,7.8V16.2A5.8,5.8 0 0,1 16.2,22H7.8C4.6,22 2,19.4 2,16.2V7.8A5.8,5.8 0 0,1 7.8,2M7.6,4A3.6,3.6 0 0,0 4,7.6V16.4C4,18.39 5.61,20 7.6,20H16.4A3.6,3.6 0 0,0 20,16.4V7.6C20,5.61 18.39,4 16.4,4H7.6M17.25,5.5A1.25,1.25 0 0,1 18.5,6.75A1.25,1.25 0 0,1 17.25,8A1.25,1.25 0 0,1 16,6.75A1.25,1.25 0 0,1 17.25,5.5M12,7A5,5 0 0,1 17,12A5,5 0 0,1 12,17A5,5 0 0,1 7,12A5,5 0 0,1 12,7M12,9A3,3 0 0,0 9,12A3,3 0 0,0 12,15A3,3 0 0,0 15,12A3,3 0 0,0 12,9Z"
                        style="fill:currentColor"
                    />
                </svg>
            </a>
        </li>-->
</ul>
